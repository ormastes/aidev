...
run_unit_tests() {
    ...
    if [ "$COVERAGE" = true ]; then
        npx jest -c "$jest_config" --testMatch="$test_pattern" --coverage --coverageDirectory="$TEST_REPORT_DIR/coverage/$theme_name"
    else
        npx jest -c "$jest_config" --testMatch="$test_pattern"
    fi
...
run_system_tests() {
    ...
    if [ -n "$feature_files" ]; then
        ...
        if [ "$VIRTUAL_MODE" = true ]; then
            ...
            run_in_virtual_env "npx cucumber-js --config cucumber.yml --profile system --require '$theme_path/**/step_definitions/*.ts'"
        else
            npx cucumber-js --config cucumber.yml --profile system --require "$theme_path/**/step_definitions/*.ts"
        fi
    ...
    if [ -n "$stest_files" ]; then
        ...
        for test_file in $stest_files; do
            ...
            if [ "$VIRTUAL_MODE" = true ]; then
                run_in_virtual_env "npx jest --testMatch='$test_file'"
            else
                npx jest --testMatch="$test_file"
            fi
        done
    else
...
main() {
    ...
    if [ "$COVERAGE" = true ]; then
        ...
        npx nyc report --reporter=html --reporter=text --report-dir="$TEST_REPORT_DIR/coverage/combined"
    fi
