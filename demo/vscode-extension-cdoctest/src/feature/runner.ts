...
export async function runner(run_args: string[] | undefined, buildDirectory: string, isUseFile: boolean, resultFile: string, config: Config, 
    ...
    return new Promise(async (resolve, reject) => {
        ...
        if (config.useCmakeTarget) {
            ...
            // Check if we should build based on configuration type and buildBeforeTest setting
            let shouldBuild = true;
            if (config.type === ConfigType.Config) {
                shouldBuild = config.buildBeforeTest;
            } else if (config.type === ConfigType.ExeConfig) {
                shouldBuild = config.exe_buildBeforeTest;
            } else if (config.type === ConfigType.BinConfig) {
                shouldBuild = config.bin_buildBeforeTest;
            }
            
            if (shouldBuild) {
                try {
                    // Trigger the build—builds the active target
                    console.log('Building project before test execution...');
                ...
                console.log('Bypassing build - running tests with existing executables');
            }
        }
