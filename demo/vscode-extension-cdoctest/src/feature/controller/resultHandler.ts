...
export function getTestRunHandler( test: vscode.TestItem, config: Config, run: vscode.TestRun,  resolve: () => void): ((result: string) => void) {
    return async (result: string): Promise<void> => {
...
export async function loadDetailedCoverageHandler(testRun: vscode.TestRun, coverage: vscode.FileCoverage): Promise<vscode.FileCoverageDetail[]> {
    // Import the function from coverageHandler
    const { loadDetailedCoverage } = await import('../coverageHandler');
    return loadDetailedCoverage(coverage);
};
