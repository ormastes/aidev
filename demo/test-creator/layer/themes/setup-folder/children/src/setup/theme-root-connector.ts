...
`;
    ...
    await fs.writeFile(scriptPath, coverageScript);
    await fs.chmod(scriptPath, 0o755);
  }

  /**
   * Updates root test configuration to include the theme
   */
  private static async updateRootTestConfig(config: ThemeConnectionConfig): Promise<void> {
    const registryPath = path.join(config.rootPath, 'gen', 'themes-registry.json');
...
`;
    ...
    await fs.writeFile(path.join(config.themePath, 'jest.config.js'), jestConfig);

    // Create global setup file
    const globalSetup = `module.exports = async () => {
  console.log('Theme: ${config.themeName} (${config.themeType})');
  console.log('Connected to root: ${config.rootPath}');
};
`;

    await fs.writeFile(path.join(config.themePath, 'jest.global-setup.js'), globalSetup);
...
`;
    ...
    await fs.writeFile(path.join(config.themePath, 'jest-root-reporter.js'), reporter);
  }
}