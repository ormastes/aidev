...
`;
    ...
    await fs.writeFile(path.join(this.deployDir, 'src', 'server.ts'), serverTs);
  }
...
`;
    ...
    await fs.ensureDir(path.join(this.deployDir, 'src'));
    await fs.writeFile(path.join(this.deployDir, 'src', 'mcp-server.ts'), mcpServerScript);
...
`;
    ...
    await fs.writeFile(path.join(this.deployDir, 'setup-mcp-production.sh'), setupScript);
...
`;
    ...
    await fs.ensureDir(path.join(this.deployDir, 'src'));
    await fs.writeFile(path.join(this.deployDir, 'src', 'index.ts'), indexTs);
  }
...
`;
    ...
    await fs.writeFile(path.join(this.deployDir, 'docker-compose.yml'), dockerCompose);
  }
...
`;
    ...
    await fs.writeFile(path.join(this.deployDir, `${this.appName}.service`), service);
  }
...
`;
    ...
    await fs.writeFile(path.join(this.deployDir, 'nginx.conf'), nginxConfig);
  }
  ...
  private async saveDbPassword(): Promise<void> {
    ...
    await fs.writeFile(path.join(secretsDir, 'db_password.txt'), this.dbPassword);
