Feature: Coordinator-vllm LLM Agent Manual Tests
  As an AI system operator
  I want to manually test the coordinator-vllm agent
  So that I can ensure proper AI agent behavior

  @manual @agent-setup
  Scenario: Manual agent configuration verification
    Given the coordinator-vllm agent is configured
  ...
  Scenario: Manual agent behavior testing
    Given the coordinator-vllm agent is running
