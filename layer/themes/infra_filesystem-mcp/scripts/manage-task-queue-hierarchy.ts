...
interface TaskQueueRegistry {
  ...
  root_queue: QueueMetadata;
  theme_queues: Record<string, QueueMetadata[]>;
  queue_hierarchy: any;
...
class TaskQueueHierarchyManager {
  ...
  private registry: TaskQueueRegistry;
  private queueMap: Map<string, QueueMetadata> = new Map();
   ...
   */
  private extractThemeFromPath(filePath: string): string | undefined {
   ...
   */
  private determineParentPath(queuePath: string): string | null {
   ...
   */
  private updateQueueParent(queuePath: string, parentPath: string | null): void {
      ...
      try {
        ...
        if (content.queues) {
          for (const queue of Object.values(content.queues)) {
            if ((queue as any).items) {
              itemCount += (queue as any).items.length;
            }
          }
   ...
   */
  private buildHierarchyTree(queuePath: string): any {
   ...
   */
  async addQueue(queuePath: string, parentPath?: string): Promise<void> {
    const normalizedPath = '/' + queuePath.replace(/\\/g, '/');
    const fullPath = path.join(this.projectRoot, queuePath);
   ...
   */
  private displayNode(node: any, level: number): void {
    if (!node) return;
