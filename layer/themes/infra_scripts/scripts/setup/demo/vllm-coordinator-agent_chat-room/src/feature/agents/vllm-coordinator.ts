...
export class VLLMCoordinatorAgent extends BaseCoordinatorAgent {
  ...
  private getModelInfo(): string {
    const config = this.config as VLLMCoordinatorConfig;
    return `
**vLLM Coordinator Info**
- Model: ${config.vllmConfig.model} (${this.actualModelName})
- Server: ${config.vllmConfig.serverUrl || 'http://localhost:8000'}
- Streaming: ${config.vllmConfig.streaming ? 'enabled' : 'disabled'}
- Temperature: ${this.modelConfig.temperature}
- Max Tokens: ${this.modelConfig.maxTokens}
- Context Size: ${config.vllmConfig.contextSize || 10} messages
