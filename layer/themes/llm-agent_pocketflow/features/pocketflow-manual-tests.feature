Feature: Pocketflow LLM Agent Manual Tests
  As an AI system operator
  I want to manually test the pocketflow agent
  So that I can ensure proper AI agent behavior

  @manual @agent-setup
  Scenario: Manual agent configuration verification
    Given the pocketflow agent is configured
  ...
  Scenario: Manual agent behavior testing
    Given the pocketflow agent is running
