...
export default defineConfig({
  test: {
    ...
    include: ['test/**/*.{test,spec}.{js,mjs,cjs,ts,mts,cts,jsx,tsx}']
      ...
      await fileAPI.createFile(path.join(this.deployDir, 'vitest.config.ts'), { type: FileType.TEMPORARY }));
  },
...
`;
    ...
    await fileAPI.createFile(path.join(this.deployDir, 'test', { type: FileType.TEMPORARY });
  }
...
export default defineConfig({
  ...
  webServer: {
    ...
    await fileAPI.createFile(path.join(this.deployDir, 'playwright.config.ts'), { type: FileType.TEMPORARY }) => {
...
describe('Task Service', () => {
  ...
  async printSuccessMessage(): void {
    const port = this.getPortAllocation();
