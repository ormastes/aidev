...
exports.FileStructureValidator = void 0;
...
class FileStructureValidator {
     ...
     */
    async loadFileStructure(structurePath) {
    ...
    async validate(targetPath) {
        ...
        if (this.fileStructure.templates.workspace) {
            await this.validateTemplate(pathToValidate, this.fileStructure.templates.workspace, 'root');
        }
        // Validate themes
     ...
     */
    async validateTemplate(dirPath, template, context) {
        this.totalChecks++;
        // Check freeze rules
        if (template.freeze) {
     ...
     */
    async validateChild(parentPath, child, required, context) {
        this.totalChecks++;
     ...
     */
    async checkFreezeViolations(dirPath, template) {
     ...
     */
    async checkUnexpectedFiles(dirPath, template) {
        // Already handled in checkFreezeViolations
    }
    /**
     * Check pattern matching
     */
    async checkPatterns(dirPath, patterns) {
        if (!fs.existsSync(dirPath))
            return;
        const items = fs.readdirSync(dirPath);
        for (const item of items) {
     ...
     * Validate all themes
     */
    async validateThemes(themesPath) {
    ...
    generateReport() {
        ...
        const complianceScore = this.totalChecks > 0
            ...
            : 100;
        const suggestions = Array.from(new Set(this.violations.map(v => v.suggestion).filter(s => s)));
        return {
            timestamp: new Date().toISOString(),
            version: ((_a = this.fileStructure) === null || _a === void 0 ? void 0 : _a.metadata.version) || '1.0.0',
...
exports.FileStructureValidator = FileStructureValidator;
exports.default = FileStructureValidator;
