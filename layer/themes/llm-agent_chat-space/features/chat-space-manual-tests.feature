Feature: Chat-space LLM Agent Manual Tests
  As an AI system operator
  I want to manually test the chat-space agent
  So that I can ensure proper AI agent behavior

  @manual @agent-setup
  Scenario: Manual agent configuration verification
    Given the chat-space agent is configured
  ...
  Scenario: Manual agent behavior testing
    Given the chat-space agent is running
