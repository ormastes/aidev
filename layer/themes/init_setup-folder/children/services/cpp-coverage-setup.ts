...
export class CppCoverageSetup {
  ...
  async setup(projectPath: string, config?: Partial<CppCoverageConfig>): Promise<void> {
    ...
    console.log(`🔧 Setting up C++ coverage for project: ${projectPath}`);
...
`;
  ...
  private async printUsageInstructions(projectPath: string): void {
    console.log(`
📝 To use coverage in your CMake project:
...
📊 Coverage reports will be in: build/coverage/
    `);
  }
}
...
export class CppCoverageChecker {
  ...
  private async validateThresholds(coverageData: any, thresholds: CppCoverageConfig['thresholds']): boolean {
    ...
    if (metrics.line < thresholds.line) {
      passed = false;
      results.push(`❌ Line coverage ${metrics.line.toFixed(2)}% < ${thresholds.line}%`);
    } else {
      results.push(`✅ Line coverage ${metrics.line.toFixed(2)}% >= ${thresholds.line}%`);
    }
    ...
    if (metrics.branch < thresholds.branch) {
      passed = false;
      results.push(`❌ Branch coverage ${metrics.branch.toFixed(2)}% < ${thresholds.branch}%`);
    } else {
      results.push(`✅ Branch coverage ${metrics.branch.toFixed(2)}% >= ${thresholds.branch}%`);
    }
    ...
    if (metrics.function < thresholds.function) {
      passed = false;
      results.push(`❌ Function coverage ${metrics.function.toFixed(2)}% < ${thresholds.function}%`);
    } else {
      results.push(`✅ Function coverage ${metrics.function.toFixed(2)}% >= ${thresholds.function}%`);
    }
