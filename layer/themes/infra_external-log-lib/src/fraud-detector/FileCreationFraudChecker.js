...
exports.FileCreationFraudChecker = void 0;
...
class FileCreationFraudChecker {
    constructor(basePath = process.cwd()) {
        this.violations = new Map();
        this.basePath = basePath;
        this.fileAPI = new FileCreationAPI_1.FileCreationAPI(basePath, false);
    ...
    async scanFile(filePath) {
        ...
        const isExempt = (0, enforcement_config_1.isDirectFSAllowed)(relativePath);
        ...
        const result = {
            ...
            violations,
            suggestions,
            canAutoFix: violations.some(v => this.patterns.find(p => p.name === v.pattern)?.autoFix || false)
        };
     ...
     */
    generateFixSuggestion(patternName, code) {
        switch (patternName) {
            case 'direct_fs_writeFile':
                return code.replace(/fs\.writeFile(Sync)?\s*\(/, 'await fileAPI.createFile(');
            case 'direct_fs_promises_writeFile':
                return code.replace(/fs\.promises\.writeFile\s*\(/, 'await fileAPI.createFile(');
            case 'direct_fs_appendFile':
                return code.replace(/fs\.appendFile(Sync)?\s*\(/, 'await fileAPI.writeFile(');
            case 'direct_fs_mkdir':
                return code.replace(/fs\.mkdir(Sync)?\s*\(/, 'await fileAPI.createDirectory(');
            case 'deno_writeTextFile':
                return code.replace(/Deno\.writeTextFile\s*\(/, 'await fileAPI.createFile(');
            default:
    ...
    async autoFix(filePath) {
        ...
        if (modified) {
            ...
            const hasImport = lines.some(line => line.includes('FileCreationAPI') || line.includes('fileAPI'));
            if (!hasImport) {
    ...
    async generateReport(options = {}) {
        ...
        const violationsBySeverity = new Map();
     ...
     */
    addViolationSection(lines, severity, results) {
     ...
     */
    async exportViolations(outputPath) {
        ...
        await this.fileAPI.createFile(exportPath, JSON.stringify(data, null, 2), {
            type: FileCreationAPI_1.FileType.DATA
        });
        return exportPath;
    }
    /**
...
exports.FileCreationFraudChecker = FileCreationFraudChecker;
exports.default = FileCreationFraudChecker;
//# sourceMappingURL=FileCreationFraudChecker.js.map