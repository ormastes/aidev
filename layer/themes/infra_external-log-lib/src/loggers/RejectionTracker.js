...
exports.RejectionTracker = exports.RejectionType = void 0;
...
(function (RejectionType) {
    RejectionType["FILE_VIOLATION"] = "file_violation";
    RejectionType["PERMISSION_DENIED"] = "permission_denied";
    RejectionType["VALIDATION_FAILED"] = "validation_failed";
    RejectionType["QUOTA_EXCEEDED"] = "quota_exceeded";
    RejectionType["FREEZE_VIOLATION"] = "freeze_violation";
    RejectionType["PATTERN_MISMATCH"] = "pattern_mismatch";
    RejectionType["DUPLICATE_FILE"] = "duplicate_file";
    RejectionType["BACKUP_FILE"] = "backup_file";
    RejectionType["UNEXPECTED_DIRECTORY"] = "unexpected_directory";
    RejectionType["MISSING_REQUIRED"] = "missing_required";
    RejectionType["SYSTEM_ERROR"] = "system_error";
})(RejectionType || (exports.RejectionType = RejectionType = {}));
class RejectionTracker extends events_1.EventEmitter {
    constructor(config) {
        super();
        this.rejections = new Map();
        this.rejectionHistory = [];
        this.pathRejectionCount = new Map();
     ...
     * Track a rejection
     */
    async trackRejection(type, reason, details) {
        const rejection = {
        ...
        if (rejection.path) {
            ...
            if (count + 1 >= 5) {
                this.logger.logEvent(EventLogger_1.LogEventType.EVENT_WARNING, `Path ${rejection.path} has ${count + 1} rejections`, { path: rejection.path, count: count + 1 });
            }
        }
        // Log the rejection
        this.logger.logRejection(this.mapRejectionTypeToLogType(type), reason, {
            id: rejection.id,
            path: rejection.path,
     ...
     */
    async trackFileViolation(error, operation) {
     ...
     * Resolve a rejection
     */
    async resolveRejection(id, notes) {
        ...
        // Log resolution
        this.logger.logEvent(EventLogger_1.LogEventType.EVENT_CUSTOM, `Rejection ${id} resolved`, {
            id,
            type: rejection.type,
            path: rejection.path,
            notes
    ...
    async clearResolved() {
        ...
        this.logger.logEvent(EventLogger_1.LogEventType.LOG_CLEANUP, `Cleared ${resolved.length} resolved rejections`);
     ...
     */
    async mapRejectionTypeToLogType(type) {
        switch (type) {
            case RejectionType.FILE_VIOLATION:
            case RejectionType.FREEZE_VIOLATION:
            case RejectionType.PATTERN_MISMATCH:
     ...
     */
    async mapViolationTypeToRejectionType(violationType) {
     ...
     */
    async setupAutoResolve(rejection) {
     ...
     */
    async enforceMaxRejections() {
        if (this.rejections.size > this.config.maxRejections) {
            // Remove oldest resolved rejections first
            const resolved = Array.from(this.rejections.values())
                .filter(r => r.resolved)
                .sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
            const toRemove = Math.min(resolved.length, this.rejections.size - this.config.maxRejections);
    ...
    async persistRejections() {
        try {
            ...
            await fileAPI.createFile(this.config.rejectionFilePath, JSON.stringify(data, { type: pipe_1.FileType.TEMPORARY }));
        }
        catch (error) {
     ...
     */
    async loadPersistedRejections() {
        try {
            const fs = require('fs');
            if (!fs.existsSync(this.config.rejectionFilePath)) {
                return;
            }
            const data = JSON.parse(fs.readFileSync(this.config.rejectionFilePath, 'utf8'));
            if (data.rejections) {
            ...
            this.logger.logEvent(EventLogger_1.LogEventType.EVENT_CUSTOM, `Loaded ${this.rejections.size} persisted rejections`);
        }
        catch (error) {
    ...
    async setupProcessHooks() {
        ...
        process.on('unhandledRejection', (reason, promise) => {
            this.trackRejection(RejectionType.SYSTEM_ERROR, 'Unhandled promise rejection', {
                additionalInfo: {
    ...
    async close() {
        ...
        this.logger.logEvent(EventLogger_1.LogEventType.EVENT_CUSTOM, 'RejectionTracker closing', { report });
