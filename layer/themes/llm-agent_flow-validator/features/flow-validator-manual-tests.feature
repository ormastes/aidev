Feature: Flow-validator LLM Agent Manual Tests
  As an AI system operator
  I want to manually test the flow-validator agent
  So that I can ensure proper AI agent behavior

  @manual @agent-setup
  Scenario: Manual agent configuration verification
    Given the flow-validator agent is configured
  ...
  Scenario: Manual agent behavior testing
    Given the flow-validator agent is running
