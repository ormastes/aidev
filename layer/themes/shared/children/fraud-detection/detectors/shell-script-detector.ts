...
export class ShellScriptDetector implements FraudDetector {
  ...
  async detect(input: any, context?: FraudContext): Promise<FraudCheckResult> {
    ...
    let score = 0;
    let hasMiddleOrHigherSeverity = false;
    ...
    if (isShellScript) {
      ...
      if (lines > this.MAX_ALLOWED_LINES) {
        ...
        // Check if severity is MEDIUM or higher
        if (severity === FraudSeverity.MEDIUM || 
            severity === FraudSeverity.HIGH || 
            severity === FraudSeverity.CRITICAL) {
          hasMiddleOrHigherSeverity = true;
        }
      ...
      if (complexPatterns.length > 0) {
        ...
        hasMiddleOrHigherSeverity = true; // Complex patterns always trigger MEDIUM severity
        violations.push({
          type: ViolationType.SUSPICIOUS_PATTERN,
    ...
    // Fail if any violation has MEDIUM severity or higher
    const passed = violations.length === 0 || !hasMiddleOrHigherSeverity;
  ...
  private isShellScript(input: any, context?: FraudContext): boolean {
